...
public class App 
{
	...
	static LinkedHashSet<Entity> set = new LinkedHashSet<>();
    ...
    public static void main( String[] args )
    {
		...
		while(choice != 6) {
			...
			System.out.println("4. Assignments Category Menu");
			...
			case 1:
				...
				while(c != 4) {
					...
					case 4:
						System.out.println("Succesfully exited from 'Student Menu'");
						System.out.println("*****************************************************************************");
						break;
					
					default:
						System.out.println("Try again, you entered wrong choice.");
						break;
					}
				}
				break;
			...
			case 4:
				...
				while(ch != 3) {
					System.out.println();
					System.out.println("*****Assignment Category Menu*****");
					...
					case 1:
						...
						String name = sc.next();
						System.out.println("Enter subject name to add assignment category in it: ");
						sc.nextLine();
					...
					case 2:
						...
						System.out.println("Enter subject name to delete assignment category: ");
						sc.nextLine();
					...
					case 3:
						System.out.println("Successfully exited from 'Assignment category menu'");
						System.out.println("*****************************************************************************");
						break;
	...
	private static void viewData(String sname) {
		try {
			...
			System.out.printf("%-20s %-20s %-20s %-10s", "Subject", "Assignment Category", "Date of Submission", "Marks");
			System.out.println();
			for(Entity a : set) {
	...
	private static void addAssignmentCategory(String name, String subject, String assigncategory, String assigncategory2, int marks) {
		try {
			Entity a = new Entity(name, subject, assigncategory, assigncategory2, marks);
	...
	private static void displayAssignmentCategory() {
		...
		for(Entity assignment : set) {
			System.out.printf("%-15s %-20s %-20s %-20s %-10s", assignment.studentName, assignment.subject, 
								assignment.assignmentCategory, assignment.date, assignment.points);
			System.out.println();
		}
	}
	...
	private static void displaySubjectWise(String subjectName) {
		...
		System.out.printf("%-25s%-15s %-15s %-15s %-15s %-15s", "Student Name", "Test Score", "Quiz Score", 
							"Lab Score", "Project Score", "Overall Rating(%)");
		System.out.println();
	...
	private static void calculateScore(LinkedHashSet<Entity> set) {
		for(Entity key : set) {
	...
	private static void validateAssignmentCategory(Entity entity) {
		Key1 key = new Key1(entity.getStudentName(), entity.getSubject(), entity.getAssignmentCategory());

		if(entity.assignmentCategory.equalsIgnoreCase("test")){
			if(test.containsKey(key)) {
				test.put(key, test.get(key) + 1);
				entity.setAssignmentCategory("test_" + test.get(key));
			}
			else {
				test.put(key, 1);
				entity.setAssignmentCategory("test_" + test.get(key));
		...
		else if(entity.assignmentCategory.equalsIgnoreCase("quiz")){
			if(quiz.containsKey(key)) {
				quiz.put(key,quiz.get(key) + 1);
				entity.setAssignmentCategory("quiz_" + quiz.get(key));
			}
			else {
				quiz.put(key, 1);
				entity.setAssignmentCategory("quiz_" + quiz.get(key));
		...
		else if(entity.assignmentCategory.equalsIgnoreCase("lab")){
			if(lab.containsKey(key)) {
				lab.put(key,lab.get(key) + 1);
				entity.setAssignmentCategory("lab_" + lab.get(key));
			}
			else {	
				lab.put(key, 1);
				entity.setAssignmentCategory("lab_" + lab.get(key));
		...
		else if(entity.assignmentCategory.equalsIgnoreCase("project")){
			if(project.containsKey(key)) {
				project.put(key,project.get(key) + 1);
				entity.setAssignmentCategory("project_" + project.get(key));
			}
			else {
				project.put(key, 1);
				entity.setAssignmentCategory("project_" + project.get(key));
		...
		set.add(new Entity(entity.getStudentName(), entity.getSubject(), entity.getAssignmentCategory(),
				entity.getDate(), entity.getPoints()));
	}
}